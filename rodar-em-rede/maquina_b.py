...
class Valentao:
    ...
    def mostrar_estado(self):
        ...
        for processo in self.processos:
            ...
            if (
                processo.pid == self.coordenador
                and processo.ativo
            ):
                print(
                    f"{processo.pid} - {status} (Coordenador)"
                )
            else:
                print(
                    f"{processo.pid} - {status}"
                )
    ...
    def iniciar_eleicao(self, pid):
        ...
        print(
            f"Processo {vencedor} respondeu"
        )

        time.sleep(TEMPO)

        if vencedor in [2, 4, 6, 8]:
...
print(
    "\nProcesso 6 detectou a falha."
)

sistema.iniciar_eleicao(6)

sistema.mostrar_estado()

time.sleep(TEMPO)

sistema.reativar_processo(8)

time.sleep(TEMPO)

print(
    "\nProcesso 8 iniciou nova eleição."
)
